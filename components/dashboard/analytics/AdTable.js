...
export default function AdTable({ data, meta, capabilities, loading, error, onSelectAd }) {
  ...
  const mappedData = (data || []).map((item) => {
    ...
    return {
      id: item.ad_id || item.adId || item.name,
          ...
          <table className="w-full text-left">
            ...
            <tbody>
              {sorted.map((item, idx) => (
                <tr
                  key={item.id}
                  onClick={() => onSelectAd && onSelectAd(item)}
                  className={`border-b border-slate-50 hover:bg-pink-50/40 transition-colors ${
                    onSelectAd ? "cursor-pointer" : ""
                  } ${idx % 2 === 1 ? "bg-slate-50/30" : ""}`}
                >
                  <td className="py-3 pr-4 text-[13px] font-medium text-slate-800 max-w-[200px] truncate hover:text-pink-600 transition-colors">
                    {item.name}
                  </td>
